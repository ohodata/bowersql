...
public final class QuickLZ
{
    ...
    public static final String EXT = ".qp";  
	...
	public static void compress(String path, int level) throws Exception
	{
		Date dt1=new Date();
	    System.out.println("zip is start!"+dt1.getTime());
		File file = new File(path);
        BufferedInputStream bis=new BufferedInputStream(new FileInputStream(file));
        BufferedOutputStream bos=new BufferedOutputStream(new FileOutputStream(file.getPath() + EXT));
        byte data[] = new byte[BUFFER]; 
        while(bis.read(data) != -1) { 
            bos.write(compress(data,level)); 
        }
        bos.flush();
        bis.close();  
        bos.close();  
	...
	{
		Date dt1=new Date();
	    System.out.println("unzip is start!"+dt1.getTime());
		File file = new File(path);
        BufferedInputStream bis=new BufferedInputStream(new FileInputStream(file));
        BufferedOutputStream bos=new BufferedOutputStream(new FileOutputStream(file.getPath().replaceFirst(EXT, "")));
        byte data[] = new byte[BUFFER]; 
        while(bis.read(data) != -1) { 
            bos.write(decompress(data)); 
        }
        bos.flush();
        bis.close();  
        bos.close();  
